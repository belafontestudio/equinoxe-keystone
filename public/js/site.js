...
$(document).ready(function() {
    ...
    $("#sidr a.enquire-button").click(function(){
      $.sidr("close");
...
function fillGrid(data,template,q){
    ...
    if(list.total > 0){
      ...
      if(pathname == "/yachts/Charter"){
        if(lang_is== "it"){
          text = '<p id="no-yacht-found">Siamo spiacenti, ma non siamo riusciti a trovare uno yacht che soddisfa le vostre esigenze. <br> Modificare i filtri per provare una nuova ricerca.</p>';
        }else if(lang_is== "en"){
          text = '<p id="no-yacht-found">We’re sorry, but we couldn’t find a yacht meeting your requirements.<br><br>Change filters to try a new search.</p>';
        }else{
          text = '<p id="no-yacht-found">We’re sorry, but we couldn’t find a yacht meeting your requirements.<br><br>Change filters to try a new search.</p>';
        }
        data_grid.html(text).fadeIn(500);
      }
      if(pathname == "/yachts/Sale"){
        if(lang_is== "it"){
          text = '<p id="no-yacht-found">Siamo spiacenti, ma non siamo riusciti a trovare uno yacht che soddisfi le vostre esigenze. <br> Modificare i filtri di provare una nuova ricerca.</p>';
        }else if(lang_is== "en"){
          text = '<p id="no-yacht-found">We’re sorry, but we couldn’t find a yacht meeting your requirements.<br><br>Change filters to try a new search.</p>';
        }else{
          text = '<p id="no-yacht-found">We’re sorry, but we couldn’t find a yacht meeting your requirements.<br><br>Change filters to try a new search.</p>';
      ...
      if(pathname == "/expeditions"){
        if(lang_is== "it"){
          text = '<p id="no-yacht-found">Siamo spiacenti, ma non siamo riusciti a trovare alcuna spedizione che soddisfi le vostre esigenze.<br> Cambiare i filtri di provare una nuova ricerca.</p>';
        }else if(lang_is== "en"){
          text = '<p id="no-yacht-found">We’re sorry, but we couldn’t find any expedition meeting your requirements.<br><br>Change filters to try a new search.</p>';
        }else{
          text = '<p id="no-yacht-found">We’re sorry, but we couldn’t find any expedition meeting your requirements.<br><br>Change filters to try a new search.</p>';
        }
        data_grid.html(text).fadeIn(500);
...
function submitModals(){
    $("#enquire_form").submit(function()
    {
        ...
        $.ajax({
            type: "POST",
            url: "/api/mail/send?t="+tomail+"&e="+email+"&n="+name+"&s="+surname+"&m="+mail_text
        })
...
$("#yacht_modal").submit(function()
    {
        ...
        tomail = "yachts@equinoxe.it";
        var msg = $(".modal_msg").val(); // get message field value
        var mail_text = "yacht request: "+theyacht +"   "+name+"  "+surname+"   "+number+"  "+msg;
        $.ajax({
            type: "POST",
